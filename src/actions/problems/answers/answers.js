  ...
  database
    ...
    .then(async () => {
      const author = (await database
        .ref(`courses/${courseId}/problems/${problemId}/author`)
        .once('value')).val();
...
export const editAnswer = (courseId, problemId, answer) => () => {
  database
    .ref(`courses/${courseId}/problems/${problemId}/answers/${answer.id}`)
    .update({
...
export const deleteAnswer = (courseId, problemId, answerId) => () => {
  return database
    .ref(`courses/${courseId}/problems/${problemId}/answers/${answerId}`)
    .remove()
    .then(() => {
      console.log('removed');
    });
...
export const markAnswerAsCorrect = (courseId, problemId, answerId) => () =>
  database
    .ref(`courses/${courseId}/problems/${problemId}/answerId`)
    .set(answerId);
